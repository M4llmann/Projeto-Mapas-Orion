...
const TesteScreen = () => {
  ...
  const [region, setRegion] = useState(initialRegion); // Estado para a região do mapa
  ...
  const fetchUserPropriedadeId = async () => {
    try {
      // Certifique-se de usar a função `collection` corretamente com a versão modular
      const propriedadesRef = collection(db, "Propriedades");
      const q = query(propriedadesRef, where("id", "==", auth.user.uid)); // Aqui, você já usa `query` com `where`
